...
| Date | Intervention |
|---|---|
| 2026-06-10 | **Catalogue Hub Windmill copié** dans `phase-0/elytras/data/windmill_hub/` : `actions.json` (710 scripts d'action, 65 apps — top par vues de hub.windmill.dev + salesforce), `triggers.json` (29 triggers, complet), `integrations.json` (210 apps officielles, extraites de `APP_TO_ICON_COMPONENT` du repo windmill). Outil de (ré)import/fusion : `tools/import_hub_catalog.py` (gère réponses tronquées/enveloppées, dédup par ask_id ; l'API hub `scripts/top?app=…&kind=…` permet de compléter app par app — pas d'offset). |
